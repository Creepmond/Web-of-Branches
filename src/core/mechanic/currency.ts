...
class Currency {
   static time: Currency
   static seed: Currency

   get value(): Decimal {
      throw new Error("Currency 'value' is not implemented")
   }

   set value(value: Constant) {
      throw new Error("Currency 'value' is not implemented" )
   }
   ...
   bumpTo(value: Decimal) {
      this.value = Decimal.max(this.value, value);
   }

   dropTo(value: Decimal) {
